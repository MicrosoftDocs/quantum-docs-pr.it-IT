...
Prepara lo stato Choi-Jamiłkowski per una determinata operazione sui registri di riferimento e di destinazione specificati.
...
Operazione $ \Lambda $ la cui proprietà Choi – Jamiłkowski state $J (\Lambda)/2 ^ N $ deve essere preparata, dove $N $ è il numero di qubits su cui `op` agisce.

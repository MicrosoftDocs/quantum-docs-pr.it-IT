...
Il valore della proprietà `version` deve essere `"0.2"`.
...
L'oggetto Broombridge deve avere una proprietà `problem_description` il cui valore è una matrice JSON.
Ogni elemento del valore della proprietà `problem_description` deve essere un oggetto JSON che descrive un set di integrali, come descritto nella parte restante di questa sezione.
Nella parte restante di questa sezione, il termine "oggetto Descrizione problema" si riferisce a un elemento nel valore della proprietà `problem_description` dell'oggetto Broombridge.

Ogni oggetto Descrizione problema deve avere una proprietà `metadata` il cui valore è un oggetto JSON.
...
La proprietà `hamiltonian` di ogni oggetto Descrizione problema descrive l'Hamiltoniana per un particolare problema di chimica quantistica elencando i termini di uno e due corpi come matrici di tipo sparse di numeri reali.
...
$ $ h {IJKL} = h {Ijlk} = h {jikl} = h {JILK} = h {klij} = h {klji} = h {lkij} = h {lkji}.
$$
...
Ogni oggetto Descrizione problema deve avere una proprietà `hamiltonian` il cui valore è un oggetto JSON.
...
Ogni oggetto Descrizione problema deve disporre di una proprietà `coulomb_repulsion` il cui valore è un oggetto quantità semplice.
Ogni oggetto Descrizione problema deve disporre di una proprietà `energy_offet` il cui valore è un oggetto quantità semplice.
...
> Si noti Ciò riflette la simmetria che $h _ {IJ} = h {ji} $, che è una conseguenza del fatto che l'hamiltoniana è Hermitiane.
...
La seguente quantità di matrici di tipo sparse rappresenta l'Hamiltoniana $ $ H = \left (-5,0 (a ^\{\dagger\}\_{1, \uparrow} a\_{1, \uparrow} + a ^\{\dagger\}\_{1, \downarrow} a\_{1 , \downarrow}) + 0,17 (a ^\{\dagger\}\_{2, \uparrow} a\_{1, \uparrow} + a ^\{\dagger\}\_{1, \uparrow} a\_{2, \uparrow} + a ^\{\dagger\}\_{2 , \downarrow} a\_{1, \downarrow} + a ^\{\dagger\}\_{1, \downarrow} a\_{2, \downarrow}) \right)\\, \mathrm{Ha}.
$$
...
L'oggetto `initial_state_suggestion` il cui valore è una matrice JSON specifica gli Stati del quantum iniziale di interesse per l'Hamiltoniana specificata. Ogni elemento del valore della proprietà `initial_state_suggestion` deve essere un oggetto JSON che descrive uno stato quantum, come descritto nella parte restante di questa sezione.
Nella parte restante di questa sezione, il termine "oggetto stato" fa riferimento a un elemento nel valore della proprietà `initial_state_suggestion` dell'oggetto Broombridge.

#### <a name="state-object"></a>State (oggetto) ####

Questa sezione è normativa.

Ogni oggetto stato deve avere una proprietà `label` contenente una stringa. Ogni oggetto stato deve avere una proprietà `method`. Il valore della proprietà `method` deve essere uno dei valori consentiti elencati nella tabella 3.
Ogni oggetto stato può avere una proprietà `energy` il cui valore deve essere un oggetto quantità semplice.

Se il valore della proprietà `method` è `sparse_multi_configurational`, l'oggetto stato deve avere una proprietà `superposition` contenente una matrice di Stati di base e le relative ampiezze non normalizzate.

Ad esempio, gli stati iniziali $ $ \ket{G0} = \ket{G1} = \ket{G2} = (a ^ {\dagger}\_{1, \uparrow}a ^ {\dagger}\_{2, \uparrow}a ^ {\dagger}\_{2, \downarrow}) \ket{0} $ $ $ $ \ket{E} = \frac{0.1 (a ^ {\dagger}\_{1 , \uparrow}a ^ {\dagger}\_{2, \uparrow}a ^ {\dagger}\_{2, \downarrow}) + 0.2 (a ^ {\dagger}\_{1, \uparrow}a ^ {\dagger}\_{3, \uparrow}a ^ {\dagger}\_{2, \downarrow})} {\sqrt{| 0.1 | ^ 2 + | 0.2 | ^ 2}} \ket{0}, $ $ dove $ \ket{E} $ ha energia $0,987 \textrm{Ha} $, sono rappresentati da
```yaml
...
Se il valore della proprietà `method` è `unitary_coupled_cluster`, l'oggetto stato deve avere una proprietà `cluster_operator` il cui valore è un oggetto JSON.
L'oggetto JSON deve avere una proprietà `reference_state` il cui valore è uno stato di base.
L'oggetto JSON può avere una proprietà `one_body_amplitudes` il cui valore è una matrice di operatori del cluster a corpo singolo e le relative ampiezze.
L'oggetto JSON può avere una proprietà `two_body_amplitudes` il cui valore è una matrice di operatori di cluster con due corpi e le relative ampiezze.
...
$ $ T = 0,1 a ^ {\dagger}\_{3, \uparrow}a\_{2, \downarrow} + 0,2 a ^ {\dagger}\_{2, \uparrow}a\_{2, \downarrow}-0,3 a ^ {\dagger}\_{1, \uparrow}a ^ {\dagger}\_{3, \downarrow}a\_{3 , \uparrow}a\_{2, \downarrow} $ $ è rappresentato da
```yaml
initial_state_suggestions: # optional. If not provided, spin-orbitals will be filled to minimize one-body diagonal term energies.
  - label: "UCCSD"
...
Ogni oggetto descrizione del problema può avere una proprietà `basis_set`.

...
In particolare, Let $H = \sum_{j = 1} ^ m H_j $ is the hamiltoniana.
...
Questo errore si verifica perché $e ^ {-iHt} $ è un operatore esponenziale e, di conseguenza, si verifica un errore durante l'utilizzo di questa formula a causa del fatto che i termini $H _J $ non vengono commutati (*ad esempio*, $H _J H_k \ne H_k H_j $ in generale).
...
La formula più semplice, la formula di Trotter simmetrica o la suddivisione Strang, assume il formato $ $ U_1 (t) = \prod_{j = 1} ^ m e ^ {-iH_j t/2} \ prod_ {j = m} ^ 1 e ^ {-iH_j t} = e ^ {-iHt} + O (m ^ 3 t ^ 3), $ $, che può essere reso inferiore a $ \epsilon $ per qualsiasi $ \epsilon > 0 $ scegliendo $ r $ per la scalabilità come $m ^ {3/2} t ^ {3/2}/\sqrt {\ Epsilon} $.

È possibile costruire formule Trotter anche di ordine superiore basate su $U _1 $.
Il più semplice è la formula del quarto ordine seguente: originariamente introdotta da Suzuki: $ $ U_2 (t) = U_1 ^ 2 (s_1t) U_1 ([1-4s_1] t) U_1 ^ 2 (s_1 t) = e ^ {-iHt} + O (m ^ 5T ^ 5), $ $ where $s _1 = (4-4 ^ {1/3}) ^{-1}$.
...
Questo formato di reprsentation Giordania-Wigner che può essere utilizzato dagli algoritmi di simulazione Q # è un tipo definito dall'utente `JordanWignerEncodingData`.
...
Per prima cosa, Let $H = \sum_j h_j H_j $ per Unity e Hermitiane $H _J $ e $h _J \ GE $0.
...
La prima subroutine quantistica utilizzata da qubitization viene chiamata $ \operatorname{Select} $ ed è promessa di produrre \begin{Equation} \operatorname{Select} \ket{j} \ket{\psi} = \ket{j} H_j \ket{\psi}, \end{Equation} in cui si presuppone che ogni $H _J $ sia Hermitiane e unitario.
...
L'operazione $ \operatorname{Prepare} $ non viene utilizzata direttamente in qubitization, bensì viene utilizzata per implementare una reflection sullo stato che $ \operatorname{Prepare} $ crea $ $ \begin{align} R &amp; = 1-2 \ operatorName {prepare} \ket{0}\bra @no_ _t_2_ \operatorname{Prepare} ^{-1} \\\\ &amp; = \operatorname{Prepare} \Lambda \operatorname{Prepare} ^{-1}.
\end{align} $ $
...
Un esempio che prevede la simulazione del modello Hubbard può essere trovato come [esempio Q #](https://github.com/Microsoft/Quantum/tree/master/Samples/src/HubbardSimulation).

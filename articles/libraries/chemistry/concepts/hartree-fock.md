...
Ciò è dovuto al fatto che per la maggior parte delle molecole in quantità di temperatura della stanza, ad esempio i tassi di reazione, sono dominate da differenze di energia gratuite tra gli Stati del quantum che descrivono l'inizio e la fine di un passaggio in un percorso di reazione e la temperatura intermedia lo stato è in genere stati di base.
...
```
È anche possibile indicizzare le funzioni Wave con `SpinOrbital` indici e quindi convertire questi indici in numeri interi come indicato di seguito.
```csharp

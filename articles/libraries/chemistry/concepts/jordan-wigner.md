---
title: Rappresentazione Giordania-Wigner | Microsoft Docs
description: Documentazione concettuale per la rappresentazione Giordania-Wigner
author: nathanwiebe2
...
Tenere presente che uno stato $ \ket{0}_J $ implica che la rotazione orbitale $j $ è vuota e $ \ket{1}_J $ implica che è occupata.
...
È facile verificare che \begin{align} a ^ \dagger_j & = \begin{Bmatrix}0 & 1 \\\ 0 & 0 \end{Bmatrix} = \frac{X_j + iY_j}{2}, \nonumber\\\\ a_j & = \begin{Bmatrix}0 & 0 \\1 & 0 \end{ Bmatrix} = \frac{X_j-iY_j}{2}, \end{align} dove $X _J $ e $Y _J $ sono gli operatori Pauli-$X $ e-$Y $ che operano su qubit $j $.
...
Dopo aver eseguito questa operazione (e aver richiamato $X _jY_j = iZ_j $), abbiamo \begin{Equation} n_j = a ^ \dagger_j a_j = \frac{(1-Z_j)}{2}.
\end{equation}
...
\begin{align} h {PP} A_p ^ \dagger A_p & = \sum_p \frac{h_{PP}}{2}(1-Z_p)\\\\ h {PQ} (A_p ^ \dagger a_q + a ^ \dagger_q A_p) & = \frac{h_{PQ}}{2}\left (\prod_{j = q + 1} ^ {p-1} Z_j \right) \ Left (X_pX_q + Y_pY_q\right)\\\\ h {pqqp} n_P n_q & = \frac{h_{pqqp}}{4}\left (1-Z_p-Z_q + Z_pZ_q \right)\\\\ h {pqqr} & = \frac{h_{pqqr}}{2}\left (\prod_{j = r + 1} ^ {p-1} Z_j \right) \left (X_pX_r + Y_pY_r\right) \left (\frac{1-Z_q}{2}\right)\\\\ h {PQRS} & = \frac{h_{PQRS}}{8}\prod_{j = s + 1} ^ {r-1} Z_j\prod_ {k = q + 1} ^ {p-1} Z_k \Big (XXXX-XXYY + XYXY\nonumber\\\\ & \qquad\qquad\qquad\qquad\qquad + YXXY + YXYX-YYXX\nonumber\\\\ & \qquad\qquad\qquad\qquad\qquad + XYYX + YYYY\Big) \end{align}
...
In alternativa, è possibile creare automaticamente la `JordanWignerEncoding` data una rappresentazione `FermionHamiltonian` dell'hamiltoniana.

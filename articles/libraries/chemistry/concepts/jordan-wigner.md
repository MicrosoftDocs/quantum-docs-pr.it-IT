...
A questo punto, $a ^ \ dagger_j \ket{0}_j = \ket{1}_j $ e $a ^ \ dagger_j \ket{1}_j = $0.
È facile verificare che \begin{align} a ^ \ dagger_j & = \begin{Bmatrix}0 & 0 \\\ 1 & 0 \end{Bmatrix} = \frac{X_j-iY_j}{2}\nonumber\\\\ a_j & = \begin{Bmatrix}0 & 1 \\\ 0 & 0 \end{Bmatrix} = \frac{X_j + iY_j}{2}, \end{align} in cui $X _J $ e $Y _J $ sono gli operatori Pauli-$X $ e-$Y $ che operano su qubit $j $.

>[!NOTE]
> In Q # lo stato $ \ket{0}$ rappresenta il autostato + 1 dell'operatore $Z $. In alcune aree di fisica $ \ket{0}$ rappresenta lo stato di basso consumo energetico, quindi il autostato-1 dell'operatore $Z $. Di conseguenza, alcune formule potrebbero differire dalla letteratura popolare.
...
Al termine dell'operazione, richiamando $X _jY_j = iZ_j $), abbiamo \begin{Equation} n_j = a ^ \ dagger_j a_j = \frac{(1-Z_j)}{2}.
\end{equation}

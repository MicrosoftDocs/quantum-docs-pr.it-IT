...
Come esempio della seconda quantizzazione in azione, supponiamo che $ \psi_0\cdots \psi_{N-1} $ sia un set ortonormale di orbitali spaziali.
...
Ovvero, gli Stati di base validi sono nel formato $ \psi_{0, \uparrow}, \ldots, \psi_{N-1, \uparrow}, \psi_{0, \downarrow}, \ldots, \psi_{N-1, \downarrow} $ where $ \uparrow $ e $ \downarrow $ sono etichette che specificano i due autostati del grado di rotazione di libertà.
...
Ciò significa che è possibile considerare formalmente la base sia per la parte rotativa che per quella spaziale della funzione Wave come $ \psi_{0} \cdots \psi_{2N-1} $ dove ogni indici è ora un'enumerazione di $ (j, \sigma) $.
...
Ciò significa che è possibile scrivere i due stati legali per $ \psi_1 $ come \begin{Equation} \psi_1 \rightarrow \begin{cases} \ket{0}_1 & \Text{if $ \psi_1 $ non è occupato,} \\\
...
\begin{Equation} \hat{H} = \sum\_{PQ} h\_{PQ} a ^ \dagger\_p a\_q + \frac{1}{2}\sum\_{PQRS} h\_{PQRS} a ^ \dagger\_p a ^ \dagger\_q a\_ra\_s + h\_{\textrm NUC}, \label{EQ: totalha} \end{Equation} dove $h\_{\textrm NUC} $ è l'energia nucleare, ovvero una costante sotto l'approssimazione di Oppenheimer,
...
\begin{align} h\_{PQRS} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \psi\_q ^\*(x\_2) \left (\frac{1}{| X_1-x_2 |} \ Right) \psi\_r (x\_2) \psi\_s (x\_1) \mathrm{d} ^ 3x\_1 \ mathrm {d} ^ 3x\_2. \ label {EQ: integralis} \end{align}

I termini $h\_{PQ} $ vengono indicati come integrali a un elettrone, perché tutti questi termini coinvolgono solo elettroni singoli e, allo stesso modo, $h\_{PQRS} $ sono integrali a due elettroni.
...
Ad esempio, $h _ {PQ} a ^ \dagger_p a_q $ descrive il salto degli elettroni da Orbital spin $q $ a spin Orbital $p $.
Analogamente, il termine $h _ {PQRS} a ^ \dagger_p a ^ \dagger_q a_r a_s $ (per Distinct $p, q, r, s $) descrive due elettroni negli orbitali della rotazione $r $ e $s $ scattering tra loro e che terminano con le orbite di rotazione $p $ e $q $.
Se $r = q $ e $p = s $, $h _ {PRRP} a ^ \dagger_p a ^ \dagger_r a_r A_p = h {PRRP} n_P n_r $ fornisce la penalizzazione energetica associata ai due elettroni che si avvicinano tra loro, ma non descrive un processo dinamico.
...
Prendere in considerazione l'Hamiltoniana $ \hat{H} = A_0 ^ \dagger A_1 + A_1 ^ \dagger A_0 $.
```csharp
...
Ad esempio, nel caso in cui $p, q, r, s $ siano indici distinti, è possibile usare le regole di anti-commutazione per mostrare che: $ $ a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{r} a\_{s} =-a ^ \dagger\_{q} a ^ \dagger\_{ p} a\_{r} a\_{s} =-a ^ \dagger\_{p} a ^ \dagger\_{q} a\_{s} a\_{r} = a ^ \dagger\_{q} a ^ \dagger\_{p} a\_{s} a\_{r}.
$$

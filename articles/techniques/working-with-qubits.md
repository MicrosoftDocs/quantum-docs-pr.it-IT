...
Infine, viene illustrato un esempio di modello ur per preparare uno stato quantico $ \frac{1}{\sqrt{3}} \left (\sqrt{2}\ket{0}+ \ket{1}\right) $, a partire dallo stato $ \ket{+} $. Vedere anche l' [esempio di unit test fornito con la libreria standard](https://github.com/microsoft/Quantum/blob/master/samples/diagnostics/unit-testing/RepeatUntilSuccessCircuits.qs): 
...
Le principali funzionalità programmatiche illustrate in questa operazione sono più complesse `fixup` parte del ciclo, che implica operazioni Quantum, e l'uso di istruzioni `AssertProb` per verificare la probabilità di misurare lo stato del quantum in determinati punti specificati del programma. Per ulteriori informazioni sulle istruzioni `Assert` e `AssertProb`, vedere anche [test e debug](xref:microsoft.quantum.techniques.testing-and-debugging) . 

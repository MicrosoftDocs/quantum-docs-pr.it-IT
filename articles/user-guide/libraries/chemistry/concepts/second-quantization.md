...
Anziché assegnare ogni $N _e $ elettroni a uno stato specifico (o orbitale), la seconda quantizzazione tiene traccia di ogni Orbital e archivia se è presente un elettrone presente in ognuno di essi e allo stesso tempo garantisce automaticamente le proprietà di simmetria della funzione Wave corrispondente.
Questo è importante perché consente di specificare modelli di chimica quantistica senza doversi preoccupare dell'anti-symmetrizing lo stato di input (come richiesto per fermioni) e anche perché la seconda quantizzazione consente di simulare tali modelli usando computer Quantum piccoli.
...
Ciò significa che gli Stati di base validi sono nel formato $ \ psi_ {0, \uparrow}, \ldots, \ psi_ {N-1, \uparrow}, \ psi_ {0, \downarrow}, \ldots, \ psi_ {N-1, \downarrow} $ where $ \uparrow $ e $ \downarrow $ sono etichette che specificano i due autostati del grado di libertà di rotazione.
Questo indice combinato di $ (j, \sigma) $ per $ \sigma \in \{ \uparrow, \downarrow \} $ è denominato rotazione-orbitale perché archivia sia il grado di libertà spaziale che quello di rotazione.
Nella libreria di chimica, gli orbitali di rotazione vengono archiviati in una `SpinOrbital` struttura di dati e vengono creati come indicato di seguito.
...
Ad esempio, \begin{align} a ^ \ dagger_1 \ket {0} _1 = \ket {1} _1, \quad a ^ \ dagger_1 \ket {1} _1 = 0, \quad A_1 \ket _1 {0} = 0, \quad A_1 \ket _1 {1} = \ket {0} _1.
\end{align} si noti che qui $a ^ \ dagger_1 \ket {1} _1 = 0 $ e $a _1 \ket _1 {0} $ yield the zero vector not $ \ket {0} _1 $.
Tali operatori non sono pertanto né Hermitiane né Unity.
Sono rappresentati gli operatori di creazione e annientamento generali usando il <xref:Microsoft.Quantum.Chemistry.LadderOperators.LadderOperator`1> tipo.
...
Anche usando tali operatori è possibile esprimere $ $ \ket {0} \ket {1} \ket {1} \ket {0} = a ^ \ dagger_1 a ^ \ dagger_2 \ket {0} ^ {\otimes 4}.
$ $ Questa sequenza di operatori viene costruita nella libreria di simulazione hamiltoniana usando codice C# simile al caso orbitario a rotazione singola considerato sopra:
```csharp
...
Per un sistema di $k $ fermioni, nella seconda quantizzazione l'azione dell'operatore di creazione $a ^ \ dagger_i $ viene fornita da $ $ a ^ \ dagger_i \ket{n_1, n_2, \ldots, 0_i, \ldots, n_k} = (-1) ^ {S_i} \ket{n_1, n_2, \ldots, 1_i, \ldots, n_k}, $ $ e $ $ a ^ \ dagger_i \ket{n_1, n_2, \ldots, 1_i, \ldots, n_k} = 0, $ $ where $S _i = \ sum_ {j<i} a ^ \ dagger_j a_j $ misura il numero totale di fermioni presenti nello stato di una singola particella e che hanno un indice $j < i $.
...
\end{align} simile agli esempi precedenti `FermionTerm` , questo operatore Number viene costruito come indicato di seguito.
```csharp
...
\end{align} pertanto le due <xref:Microsoft.Quantum.Chemistry.LadderOperators.LadderSequence`1> istanze seguenti sono considerate inequivalenti
```csharp
    // Let us initialize an array of tuples representing the
    // desired sequence of creation operators.
    var indices = new[] { (RaisingLowering.u, 1), (RaisingLowering.u, 2) };
...
Utilizzando le regole di anti-commutazione, alcune `LadderSequence` istanze corrispondono in realtà alla stessa sequenza di operatori fermioniche, a volte fino a un segno meno.
...
Qualsiasi `FermionTerm` viene inserito automaticamente nell'ordine canonico come indicato di seguito.
```csharp
...
\begin{align} h \_ {PQRS} &= \int \_ {-\infty} ^ \infty \int \_ {-\infty} ^ \infty\psi \_ p ^ \* (x \_ 1) \psi \_ q ^ \* (x \_ 2) \left (\frac {1} {| X_1-x_2 |} \right) \psi \_ r (x \_ 2) \psi \_ s (x \_ 1) \mathrm{d} ^ 3x \_ 1 \ mathrm {d} ^ 3x \_ 2. \ etichetta {EQ: integrali} \end{align}

I termini $h \_ {PQ} $ sono definiti integrali di un elettrone, perché tutti questi termini coinvolgono solo elettroni singoli e $h \_ {PQRS} $ sono integrali a due elettroni.
...
Questi hamiltonians possono essere rappresentati usando la `FermionHamiltonian` classe, che è essenzialmente un elenco contenente tutte le `FermionTerm` istanze desiderate.
Poiché hamiltonians sono Hermitiane per definizione, vengono indicizzati i termini usando il tipo più specializzato `HermitianFermionTerm` che usa anche la simmetria Hermitiane per verificare se i termini sono equivalenti.
...
Quando si aggiungono termini a hamiltoniana usando `Add` , si presuppone che qualsiasi termine non hermitiane, ad esempio, venga `fermionTerm0` abbinato al relativo coniugato Hermitiane.
...
Con le regole di anti-commutazione, alcune `FermionTerm` istanze nell'Hamiltoniana corrispondono in realtà alla stessa sequenza di operatori fermioniche, a volte fino a un segno meno.
...
Ad esempio, nel caso in cui $p, q, r, s $ sono indici distinti. è possibile usare le regole di anti-commutazione per illustrare quanto segue: $ $ a ^ \dagger \_ {p} a ^ \dagger \_ {q} a { \_ r} a \_ {s} =-a ^ \dagger \_ {q} a ^ \dagger \_ {p} a { \_ r} a { \_ s} =-a ^ \dagger \_ {p} a ^ \dagger {q} a {s} a { \_ \_ \_ r} = a ^ \dagger \_ {q} a ^ \dagger \_ {p} a { \_ s} a \_ {r}.
$$

Inoltre, poiché $H $ è hermitiane, ogni operatore fermioniche non hermitiane, ad esempio $h \_ {PQRS} a ^ \dagger \_ {p} a ^ \dagger \_ {q} a \_ {r} a \_ {s} $, ha un coniugato Hermitiane presente anche in $H $. Per indicizzare in modo univoco i gruppi di termini caratterizzati da queste simmetrie, viene definito un ordinamento canonico sugli indici $ (i \_ 1, \cdots, i \_ n, j \_ 1, \cdots, j \_ m) $ di qualsiasi sequenza di $n + m $ fermioniche Operators $a ^ \dagger \_ {i \_ 1} \cdots a ^ \dagger \_ {i \_ n} a \_ {j \_ 1} \cdots a \_ {j \_ m} $As seguente:
...
-   Tutti gli indici degli operatori di creazione sono ordinati in ordine crescente, ovvero $i \_ 1< i \_ 2< \cdots < i \_ n $.
-   Tutti gli indici degli operatori di annientamento sono ordinati in ordine decrescente, ovvero $j \_ 1> j \_ 2 \cdots > j \_ m $.
-   L'indice più a sinistra è minore o uguale all'indice più a destra, ovvero $i \_ 1 \ le j \_ m $.

Identifichiamo questo set di indici ordinati in modo canonico come $ $ \begin{align} (i \_ 1, \cdots, i \_ n, j \_ 1, \cdots, j \_ m) \in S \_ {n, m}.
\end{align} $ $

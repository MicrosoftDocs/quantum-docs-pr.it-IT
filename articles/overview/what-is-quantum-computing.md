...
ms.locfileid: "73443971"
---
# <a name="what-is-quantum-computing"></a>Che cos'è il calcolo quantistico?

Alcuni problemi sono talmente complessi e di un tale impatto che, anche se ogni supercomputer al mondo lavorasse su un problema in tandem, impiegherebbe comunque un tempo superiore alla vita dell'universo per risolverlo. I computer quantistici promettono di risolvere alcune delle principali sfide del pianeta in fatto di in ambiente, agricoltura, salute, energia, clima, scienza dei materiali e ambiti ancora inimmaginabili. L'impatto dei computer quantistici sarà di vasta portata e avrà un impatto paragonabile a quello della creazione del transistor nel 1947, che ha spianato la strada alla moderna economia digitale.

Il calcolo quantistico sfrutta lo speciale comportamento della fisica quantistica per offrire un nuovo e potentissimo modello di calcolo. La teoria della fisica quantistica postula che la materia, a livello quantistico, può trovarsi in una sovrapposizione di più stati classici. E questi numerosi stati interferiscono l'uno con l'altro come onde in una pozza di marea.  Si presume che lo stato della materia dopo una misura "collassi" in uno degli stati classici. Successivamente, la ripetizione della stessa misura genererà lo stesso risultato classico.  L'entanglement quantistico si verifica quando le particelle interagiscono in modo tale che lo stato quantistico di ognuna non può essere completamente descritto indipendentemente dalle altre, anche se le particelle sono fisicamente distanti.  
...
I computer classici codificano le informazioni in bit, dove ogni bit codifica 2 possibili valori, 0 o 1.  Un qubit codifica contemporaneamente due valori, 0 e 1.  Due bit classici codificano uno di 4 valori possibili, ovvero 00, 01, 10, 11, mentre 2 qubit codificano contemporaneamente qualsiasi sovrapposizione di questi 4 valori, anche se si ottiene uno solo di questi valori durante la misura.  Quattro bit codificano uno di 16 valori.  Quattro qubit codificano contemporaneamente ogni sovrapposizione di 16 valori e così via, in modo esponenziale.  100 qubit possono codificare più informazioni rispetto a quelle attualmente disponibili nei sistemi informatici più potenti al mondo.  

Inoltre, quando più qubit correlati agiscono in modo coerente, possono elaborare più opzioni contemporaneamente. In questo modo possono elaborare le informazioni in una frazione del tempo che impiegherebbero persino i sistemi non quantistici più veloci.
...
* la progettazione di un piano di controllo criogenico esclusivo con una potenza e una dissipazione di calore estremamente basse, 
...
* [Cosa è possibile fare con un computer quantistico?](xref:microsoft.quantum.overview.computers)

---
title: Come è possibile apprendere il calcolo quantistico con Q#?
description: Risorse per informazioni di base sulla matematica e sulla fisica per iniziare a usare il calcolo quantistico.
author: natke
ms.author: nakersha
ms.date: 10/23/2019
ms.topic: article
uid: microsoft.quantum.overview.learn
ms.openlocfilehash: 17fc4e7a73f93a86d981996bf8b59309bccb6e67
ms.sourcegitcommit: 6ccea4a2006a47569c4e2c2cb37001e132f17476
ms.translationtype: HT
ms.contentlocale: it-IT
ms.lasthandoff: 02/28/2020
ms.locfileid: "77907750"
---
# <a name="how-to-learn-about-quantum-computing"></a>Come è possibile apprendere il calcolo quantistico?

Informazioni per apprendere il calcolo quantistico e scrivere i primi programmi. Questa guida non è esaustiva, ma si tratta di un ottimo punto di partenza.

## <a name="getting-started-overview"></a>Panoramica dell'introduzione

[Introduzione a Microsoft Quantum Development Kit](xref:microsoft.quantum.welcome) fornisce una panoramica generale del calcolo quantistico con Q#, incluse le esercitazioni per la scrittura del primo programma Q#, le guide introduttive e l'introduzione alle librerie quantistiche Q# per lo sviluppo di programmi quantistici.

## <a name="learning-the-basics-what-do-you-need-to-know"></a>Informazioni di base: cosa è necessario sapere?

Non serve essere esperti di fisica quantistica per imparare a usare Q# e il calcolo quantistico o per iniziare a scrivere applicazioni quantistiche.

Questi concetti offriranno una valida introduzione alle informazioni di base necessarie per iniziare a scrivere il codice per programmi quantistici.  

* [Meccanica quantistica di base](xref:microsoft.quantum.concepts.intro): come appena detto, non è necessario conoscere la fisica quantistica per iniziare a scrivere codice. È tuttavia utile conoscere alcuni concetti di base relativi alla meccanica quantistica e alla relativa notazione matematica per comprendere la programmazione quantistica.

* **Algebra lineare (vettori e matrici)** : nel calcolo quantistico gli stati quantistici sono rappresentati da vettori, mentre le operazioni quantistiche sono trasformazioni lineari applicate a questi vettori.  Ecco un'[esercitazione per Jupyter Notebook sull'algebra lineare](https://github.com/microsoft/QuantumKatas/tree/master/tutorials/LinearAlgebra).  È anche possibile approfondire ulteriormente l'algebra lineare nella guida ai concetti relativi a [vettori e matrici](xref:microsoft.quantum.concepts.vectors).

* **Aritmetica complessa**: i coefficienti dei vettori di stato quantistico sono numeri complessi. È possibile comprendere alcuni concetti di base sul calcolo quantistico anche senza conoscerli, ma ben presto dovranno entrare a far parte del kit di strumenti di calcolo quantistico.  Ecco un'[esercitazione per Jupyter Notebook sull'aritmetica complessa](https://github.com/microsoft/QuantumKatas/tree/master/tutorials/ComplexArithmetic) che descrive alcune delle basi di matematica necessarie per il calcolo quantistico. 

Ora che si conoscono le nozioni di base, è possibile iniziare a imparare come scrivere i primi programmi quantistici.  È possibile procedere in molti modi:

## <a name="do-the-quantum-katas"></a>Eseguire le esercitazioni della serie Quantum Katas

[Quantum Katas](xref:microsoft.quantum.overview.katas) è una serie open source di esercitazioni autogestite per l'apprendimento dei concetti del calcolo quantistico e della programmazione Q# allo stesso tempo.  Ogni kata fa riferimento a materiali di formazione aggiuntivi che è possibile usare per apprendere i concetti di calcolo quantistico necessari per completare il kata.  

## <a name="dive-into-the-theory"></a>Approfondimenti sulla teoria

Nel caso in cui si voglia approfondire la teoria della meccanica quantistica e del calcolo quantistico, ecco un elenco di materiali utili:

* È possibile iniziare con la guida Microsoft ai [concetti del calcolo quantistico](xref:microsoft.quantum.concepts.intro), una raccolta di concetti di base per il calcolo quantistico.
* Il volume _Learn Quantum Computing with Python and Q#_ (Sarah C. Kaiser e Christopher E. Granade) costituisce un'ottima introduzione per utenti poco esperti di meccanica quantistica, ma che conoscono i principi di base della programmazione.
* Il volume _Quantum Computation and Quantum Information_ (Michael A. Nielsen, Isaac L. Chuang) è il testo più citato nel campo del calcolo quantistico ed è considerato come il testo standard sulla materia. Questo manuale presuppone un minimo di esperienza nei settori della meccanica quantistica e dell'informatica. Si tratta del testo ideale per chi desidera una rigorosa introduzione all'argomento e per chi è alla ricerca di riferimenti per i concetti avanzati.
* In MIT OpenCourseWare è disponibile un eccellente [corso online](https://www.youtube.com/watch?v=lZ3bPUKo5zc&list=PLUl4u3cNGP61-9PEhRognw5vryrSEVLPr) sulle nozioni di base della meccanica quantistica tenuto da Allan Adams. È ideale per gli sviluppatori che vogliono conoscere meglio la fisica sottostante.

## <a name="join-the-quantum-community"></a>Partecipare alla community quantistica

Per intraprendere questo percorso di apprendimento, è possibile contare su una vasta community di utenti esperti o interessati all'argomento, tutti disposti a offrire il proprio aiuto. Domande e richieste di chiarimento

* In caso di domande su Q# o sul calcolo quantistico, è possibile visitare il sito di StackExchange sul calcolo quantistico. Se non si trova la risposta cercata, è sempre possibile porre una nuova domanda. 
* Vedere anche il [blog su Q#](https://devblogs.microsoft.com/qsharp/) e il [blog di Microsoft Quantum](https://cloudblogs.microsoft.com/quantum/) per rimanere aggiornati sulle ultime novità e risorse su Q#.
* Visitare la [community di Q#](https://qsharp.community/) e il sito [Awesome Q#](https://project-awesome.org/ebraminio/awesome-qsharp) per cercare altre risorse e materiali.

 Se si vuole insegnare un corso sul calcolo quantistico, è possibile contattare [Microsoft Quantum Network](https://info.microsoft.com/LearnMoreAboutMicrosoftQuantumNetwork.html) per ricevere assistenza sul piano di formazione.  


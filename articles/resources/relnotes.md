...
## <a name="version-0152101126807"></a>Versione 0.15.2101.126807

*Data di rilascio: 29 gennaio 2021*

- Aggiunta di modelli di progetto al Q# compilatore per i file eseguibili destinati a provider IonQ e Honeywell
- Aggiornamento Q# dell'evidenziazione della sintassi del kernel per includere le modifiche alla Q# sintassi introdotta nella versione [0.15.2101125897](#version-0152101125897)
- Bugfix per supportare il passaggio di matrici come argomenti di input ai Q# programmi inviati a Quantum di Azure tramite `%azure.execute` , vedere Issue [#401](https://github.com/microsoft/iqsharp/issues/401)
- Correzione dell'errore "autorizzazione negata" rilevata usando `az` all'interno delle `iqsharp-base` Immagini Docker, vedere Issue [#404](https://github.com/microsoft/iqsharp/issues/404)

## <a name="version-0152101125897"></a>Versione 0.15.2101125897

*Data di rilascio: 26 gennaio 2021*

- Allocazione qubit semplificata, che fornisce una sintassi più pratica per l'allocazione di qubits, [vedere i dettagli nel Q# repository dei linguaggi](https://github.com/microsoft/qsharp-language/blob/main/Approved/1-implicitly-scoped-qubit-allocation.md).
- È stato creato QDK-Python repository che include `azure-quantum` , il client Python per l'invio di processi di ottimizzazione ispirati a Quantum al servizio Quantum di Azure, nonché `qdk` , incluso `qdk.chemistry` , un livello di praticità basato su Python per la Q# libreria di chimica che include la visualizzazione molecolare e la funzionalità per generare file di input per diversi pacchetti di chimica, ad esempio nwchem, Psi4 e OpenMolcas.
- Le parentesi sono ora facoltative per i tipi di operazione e funzione e per le `if` `elif` `while` istruzioni, e `until` . Le parentesi per `for` `use` le `borrow` istruzioni e sono state deprecate.
- Miglioramento delle stime di larghezza per una profondità ottimale, [vedere dettagli](https://github.com/MicrosoftDocs/quantum-docs-pr/pull/1159).
- Applica l'operazione unitaria fornita come matrice esplicita usando `ApplyUnitary` ([QuantumLibraries # 391](https://github.com/microsoft/QuantumLibraries/pull/391), contributo esterno di Dmytro Fedoriaka)
- Correzione https://github.com/microsoft/iqsharp/issues/387 attenuando l'effetto sulle prestazioni dell' Q# avvio del kernel I.

## <a name="version-0142011120240"></a>Versione 0.14.2011120240

*Data di rilascio: 25 novembre, 2020*

- Miglioramento delle prestazioni del compilatore a causa del caricamento più rapido dei riferimenti.
- È stata aggiunta una [grammatica Q# ANTLR per](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language/5_Grammar) alla Q# specifica del linguaggio.
- Lo [ `Microsoft.Quantum.Preparation` spazio dei nomi](xref:Microsoft.Quantum.Preparation) è stato aggiornato in modo da essere più coerente con la Guida di stile e i principi di progettazione delle API e per supportare gli stati misti purificati con dati aggiuntivi (vedere la [proposta](https://github.com/microsoft/QuantumLibraries/issues/344), [esaminare le note](https://github.com/microsoft/QuantumLibraries/blob/main/Design/meetings/2020/api-design-2020-11-05.md) e le richieste pull [#212](https://github.com/microsoft/QuantumLibraries/pull/212), [#322](https://github.com/microsoft/QuantumLibraries/pull/322), [#375](https://github.com/microsoft/QuantumLibraries/pull/375) [#376](https://github.com/microsoft/QuantumLibraries/pull/376)).
- Le parentesi che racchiudono espressioni di chiamata ripetute sono ora facoltative: `(Foo(x))(y)` possono essere scritte come `Foo(x)(y)` .
...
- La stima delle risorse ora emette stime di profondità e larghezza simultaneamente ottenibili oltre al numero di qubit. Per informazioni dettagliate, vedere [qui](xref:microsoft.quantum.machines.resources-estimator#metrics-reported).

Vedere l'elenco completo di richieste pull chiuse per [librerie](https://github.com/Microsoft/QuantumLibraries/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [compilatori](https://github.com/microsoft/qsharp-compiler/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [Runtime](https://github.com/microsoft/qsharp-runtime/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [esempi](https://github.com/Microsoft/Quantum/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [ Q# I](https://github.com/microsoft/iqsharp/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22) e [Kata](https://github.com/microsoft/QuantumKatas/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22).
...
- Specifica dell'annuncio e della bozza della [rappresentazione intermedia quantistica (Qir)](https://github.com/microsoft/qsharp-language/tree/main/Specifications/QIR#quantum-intermediate-representation-qir) concepita come un formato comune per i diversi front-end e back-end. Vedere anche il [post di Blog](https://devblogs.microsoft.com/qsharp/introducing-quantum-intermediate-representation-qir) su Qir.
- Lancio del nuovo [ Q# repository di linguaggio](https://github.com/microsoft/qsharp-language) contenente anche la [ Q# documentazione](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language#q-language)completa.
- Miglioramenti delle prestazioni per QuantumSimulator per i programmi che coinvolgono un numero elevato di qubits: migliore applicazione delle decisioni di fusioni dei Gate; parallelizzazione migliorata nel sistema Linux; Aggiunta pianificazione intelligente dell'esecuzione del Gate; correzioni di bug.
- Le funzionalità di IntelliSense sono ora supportate per Q# i file in Visual Studio e Visual Studio Code anche senza un file di progetto.
- Vari Q# miglioramenti di interoperabilità/Python e correzioni di bug, incluso un supporto migliore per i tipi di dati numpy.
- Miglioramenti allo spazio dei nomi Microsoft. Quantum. Arrays (vedere [Microsoft/QuantumLibraries # 313](https://github.com/microsoft/QuantumLibraries/issues/313)).
- È stato aggiunto un nuovo [esempio di ripetizione fino al](https://github.com/microsoft/Quantum/tree/main/samples/algorithms/repeat-until-success) completamento che usa solo due qubits.

Dall'ultima versione, il ramo predefinito in ognuno dei repository Open Source è stato rinominato in `main` .

Vedere l'elenco completo di richieste pull chiuse per [librerie](https://github.com/Microsoft/QuantumLibraries/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24), [compilatori](https://github.com/microsoft/qsharp-compiler/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24), [Runtime](https://github.com/microsoft/qsharp-runtime/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24), [esempi](https://github.com/Microsoft/Quantum/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24), [ Q# I](https://github.com/microsoft/iqsharp/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24) e [Kata](https://github.com/microsoft/QuantumKatas/pulls?q=is%3Apr+closed%3A2020-08-24..2020-09-24).
...
- Nuovo supporto per i modificatori di accesso in Q# . per ulteriori informazioni, vedere [modificatori di accesso](xref:microsoft.quantum.qsharp.accessmodifiers)
...
- Nuova libreria di apprendimento automatico quantistico. Per altre informazioni, vedere la [pagina della documentazione sull'apprendimento automatico quantistico](xref:microsoft.quantum.machine-learning.concepts.intro).
...
- Nuovo attributo di test per gli Q# unit test, vedere la documentazione dell'API aggiornata [qui](xref:Microsoft.Quantum.Diagnostics.Test) e i test aggiornati & Guida al debug [qui](xref:microsoft.quantum.guide.testingdebugging)
...
- Aggiornamenti per Visual Studio Code & estensioni di Visual Studio per distribuire il server di linguaggio come file eseguibile autonomo, eliminando la dipendenza della versione .NET Core SDK  
...
- Nuovo supporto per le [istruzioni di coniugazione](xref:microsoft.quantum.qsharp.conjugations#conjugations) in Q#
...
- Nuova indicizzazione per le matrici di sezionamento, [vedere le informazioni di riferimento sul linguaggio](xref:microsoft.quantum.qsharp.contextualexpressions#contextual-and-omitted-expressions) per ulteriori informazioni.
- Aggiunta di Dockerfile ospitata in [Microsoft container Registry](https://github.com/microsoft/ContainerRegistry). [ Q# per altre informazioni](https://github.com/microsoft/iqsharp/blob/main/README.md) , vedere il repository i
...
* Aggiungere gli elementi denominati per [tipi definiti dall'utente] Microsoft. Quantum. qsharp. typedeclarations # Type-Declarations).  
* È ora possibile usare i costruttori di tipi definiti dall'utente come funzioni.
* Aggiunta del supporto per le operazioni di [copia e aggiornamento](xref:microsoft.quantum.qsharp.copyandupdateexpressions#copy-and-update-expressions) e [applicazione e riassegnazione](xref:microsoft.quantum.qsharp.variabledeclarationsandreassignments#evaluate-and-reassign-statements) nei tipi definiti dall'utente.
* Il blocco di correzioni per i cicli di [ripetizione fino al completamento](xref:microsoft.quantum.qsharp.conditionalloops#repeat-statement) è ora facoltativo.
...
### <a name="no-locq-language-syntax"></a>Q# sintassi del linguaggio
Questa versione aggiunge la nuova Q# sintassi del linguaggio:
* Aggiunta di un [modo abbreviato per esprimere le specializzazioni delle operazioni quantistiche](xref:microsoft.quantum.qsharp.specializationdeclarations) (controllo e operatori aggiunti) con operatori `+`.  La sintassi precedente è deprecata.  I programmi che usano la sintassi precedente (ad esempio `: adjoint`) continueranno a funzionare, ma verrà generato un avviso in fase di compilazione.  
* Aggiungere un nuovo operatore ternario per la [copia e l'aggiornamento](xref:microsoft.quantum.qsharp.copyandupdateexpressions#copy-and-update-expressions), `w/` `<-` , può essere usato per esprimere la creazione di matrici come una modifica di una matrice esistente.
* Aggiungere l' [istruzione Apply-and-reassign](xref:microsoft.quantum.qsharp.variabledeclarationsandreassignments#evaluate-and-reassign-statements)comune, ad esempio, `+=` , `w/=` .
* Aggiunta di un modo per specificare un nome breve per gli spazi dei nomi in [direttive open](xref:microsoft.quantum.qsharp.namespaces#open-directives).
...
- Aggiunta dell'aritmetica di addizionatori di interi alla libreria Quantum Canon.  Vedere anche un notebook di Jupyter che [descrive come usare i nuovi addizionatori di interi](https://github.com/microsoft/Quantum/blob/main/samples/arithmetic/AdderExample.ipynb).
...
- È stata aggiunta la possibilità di restituire dall'interno di un' [istruzione using-and Borrowing-](xref:microsoft.quantum.qsharp.quantummemorymanagement#quantum-memory-management).
...
- Aggiunta del supporto per un nuovo tipo primitivo, BigInt, che rappresenta un intero con segno di dimensione arbitraria.  Altre informazioni su [bigint](xref:microsoft.quantum.qsharp.valueliterals#bigint-literals).

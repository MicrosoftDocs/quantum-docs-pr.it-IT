...
autore: cgranade UID: Microsoft. Quantum. Concepts. Oracles ms. Author: chgranad ms. Date: 07/11/2018 ms. Topic: Conceptual no-loc:
- "Q#"
- "$$v"
- "$$"
- "$$"
...
> [!NOTE]
>Si noti che $ z ^ { -1 } = z ^ { \dagger } = Z $ e quindi $ z ^ { f (0)-f (1) } = z ^ { f (1)-f (0) } .$

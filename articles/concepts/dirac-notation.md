...
autore: QuantumWriter UID: Microsoft. Quantum. Concepts. Dirac ms. Author: v-benbra ms. Date: 12/11/2017 ms. Topic: Conceptual no-loc:
- "Q#"
- "$$v"
- "$$"
- "$$"
...
Sono disponibili due tipi di vettori nella notazione Dirac: il vettore *Bra* e il vettore *KET* , denominati perché, quando assemblati, formano un *freno* o un prodotto interno.  Se $ \psi $ è un vettore di colonna, è possibile scriverlo nella notazione Dirac come $ \ket { \psi } $ , dove $ \ket { \cdot indica } $ che si tratta di un vettore di colonna di unità, ovvero un vettore *KET* .  Analogamente, il vettore $ \psi ^ \dagger $ di riga viene espresso come $ \bra { \psi } $ . In altre parole, $ \psi ^ \dagger $ viene ottenuto applicando una coniugazione complessa per la voce per gli elementi della trasposta di $ \psi $ . La notazione bra-ket implica direttamente che $ \braket { \psi | \psi } $ è il prodotto interno del vettore $ \psi $ con se stesso, che è per definizione $ 1 $ .  

Più in generale, se $ \psi $ e $ \phi $ sono vettori di stato quantistici, il prodotto interno è $ \braket { \phi | \psi } $ il che implica che la probabilità di misurare lo stato $ \ket { \psi } $ $ \ket { \phi } $ è $ | \braket { \phi | \psi } | ^ 2 $ .  
...
Ciò indica che $ \ket { 0 } $ e $ \ket { 1 } $ sono vettori ortogonali, ovvero $ \braket { 0 | 1 } = \braket { 1 | 0 0 } = $ .  Anche per definizione $ \braket { 0 | 0 } = \braket { 1 | 1 } = 1 $ , il che significa che i due vettori di base di calcolo possono essere chiamati anche *ortonormale*.

Queste proprietà di ortonormale saranno utili nell'esempio seguente. Se si dispone di uno stato $ \ket { \psi } = { \frac { 3 } { 5 } } \ket { 1 }  +  { \frac { 4 } { 5 0, a } } \ket { } $ causa $ \braket { | di 1 0 } = 0 $ la probabilità di misurazione $ 1 $ è 
...
Analogamente, lo stato $ \ket { p } $ per integer $ p $ rappresenta uno stato quantico che codifica nella rappresentazione binaria del numero intero $ p $ .  Se, ad esempio, si desidera esprimere il numero $ 5 $ utilizzando una codifica binaria senza segno, è possibile esprimere lo stesso nome

$$
\ket{1 } \ket { 0 } \ket { 1 } = \ket { 101 } = \ket { 5 } .
$$

All'interno di questa notazione $ \ket { 0 } $ non deve fare riferimento a uno stato single-qubit, ma piuttosto a un *Registro qubit* che archivia una codifica binaria pari a $ 0 $ .  Le differenze tra queste due notazioni sono in genere chiare dal contesto.  Questa convenzione è utile per semplificare il primo esempio che può essere scritto in uno dei modi seguenti:
...
Come altro esempio di come è possibile usare la notazione Dirac per descrivere uno stato quantico, considerare i seguenti modi equivalenti di scrivere uno stato quantum che è una superposizione uguale su ogni stringa di bit di lunghezza $ n$

$$
H ^ { \otimes n } \ket { 0 } = \frac { 1 } { 2 ^ n { /2 } } \sum _ { j = 0 } ^ { 2 ^ n-1 } \ket { j } = \ket { + } ^ { \otimes n } .
$$

Qui è possibile chiedersi perché la somma va da $ 0 $ a $ 2 ^ { n } -1 $ per $ n $ bit.  Si noti innanzitutto che sono presenti $ 2 ^ { n } $ configurazioni diverse $ che $ possono essere accettate da n bit.  È possibile notare che un bit può assumere $ 2 $ valori, ma due bit possono richiedere $ 4 $ valori e così via. In generale, ciò significa che sono presenti $ 2 ^ n $ diverse stringhe di bit, ma il valore più grande codificato in uno di essi $ 1 \cdots 1 = 2 ^ n-1 $ e quindi è il limite superiore per la somma.
Come si può notare, in questo esempio non è stato usato $ \ket { + } ^ { \otimes n } = \ket { + } $ in Analogy a $ \ket { 0 } ^ { \otimes n } = \ket { 0 } $ perché questa convenzione di notazione è in genere riservata per lo stato di base computazionale con ogni qubit inizializzato su zero.  Sebbene tale convenzione sarebbe sensata in questo caso, non viene utilizzata nella letteratura per il calcolo del quantum.
...
I vettori Bra seguono una convenzione simile a vettori ket.  Ad esempio, il vettore $ \bra { \psi } \bra { \phi } $ è equivalente allo stato Vector $ \psi ^ \dagger \otimes \phi ^ \dagger = ( \psi \otimes \phi ) ^ \dagger $ . Se il vettore ket $ \ket { \psi } $ è $ \alpha \ket { 0 }  +  \beta \ket { 1, } $ la versione del vettore Bra del vettore è $ \bra { \psi } = \ket { \psi } ^ \dagger = ( \bra { 0 } \alpha ^ * + \bra { 1 } \beta ^ *) $ .

Si supponga, ad esempio, di voler calcolare la probabilità di misurare lo stato $ \ket { \psi } = \frac { 3 } { 5 } \ket { 1 }  +  \frac { 4 } { 5 } \ket { 0 } $ usando un programma Quantum per misurare gli Stati come $ \ket { + } $ o $ \ket { - } $ . Quindi, la probabilità che il dispositivo restituisca che lo stato $ sia \ket { - } $ 
...
Ketbras vengono spesso denominati proiettori perché proiettano uno stato quantico su un valore fisso.  Poiché queste operazioni non sono unite (e non mantengono neanche la norma di un vettore), non dovrebbe essere sorprendente che un computer quantum non possa applicare in modo deterministico un proiettore.  Tuttavia, i proiettori eseguono un bel lavoro per descrivere l'azione che la misura ha su uno stato quantico.  Se, ad esempio, si misura uno stato $ \ket { \psi } $ $ come 0 $ , la trasformazione risultante che lo stato subisce come risultato della misurazione è

  $$\ket{\psi}\right \frac freccia { ( \ket { 0 } \bra { 0 } ) \ket { \psi } } { | \braket { 0 | \psi } | } = \ket { 0 } ,$$

come si prevede se fosse necessario misurare lo stato e trovarlo come $ \ket { 0 } $ .  Per eseguire nuovamente l'iterazione, non è possibile applicare tali proiettori a uno stato in un computer Quantum in modo deterministico.  Al contrario, possono essere applicati in modo casuale con il risultato $ \ket { 0 } $ visualizzato con una probabilità fissa.  La probabilità di una misurazione di questo tipo può essere scritta come valore previsto del proiettore Quantum nello stato
...
Se invece si considera la misurazione del primo qubit di uno stato qubit su 1, $ $ è possibile descrivere questo processo in modo appropriato usando i proiettori e la notazione Dirac:

$$
P ( \text { primo qubit = 1 } ) = \bra { \psi } \left ( \ket { 1 } \bra { 1 } \otimes \boldone ^ { \otimes n-1 } \right ) \ket { \psi } .
$$
...
$$
P ( \text { primo qubit = 1 } ) = \psi ^ \dagger (e \_ { 10 } e \_ { 10 } ^ \dagger + e \_ { 11 } e 11 \_ { } ^ \dagger ) \psi = | e \_ { 10 } ^ \dagger \psi | ^ 2 + | e \_ { 11 } ^ \dagger \psi | ^ 2,$$

...
$ $ \begin{Bmatrix}1 \\\\ 0 \\\\ 0 \end{Bmatrix}, \begin{Bmatrix}0 \\\\ 1 \\\\ 0 \ end {Bmatrix} e \begin{Bmatrix}0 \\\\ 0 \\\\ 1 \ fine {Bmatrix} $ $
...
Per le matrici Hermitiane e unitarie, che sono essenzialmente le uniche matrici rilevate nel quantum computing, esiste un risultato generale noto come [*teorema spettrale*](https://en.wikipedia.org/wiki/Spectral_theorem), che asserisce quanto segue: per qualsiasi matrice Hermitiane o unitaria $M $, esiste una unitario $U $ tale che $M = U ^ \dagger D U $ per una matrice diagonale $D $. Inoltre, le voci diagonali di $D $ saranno gli autovalori di $M $.
...
$ $ e ^ A = U ^ \dagger \left (\boldone + D + \frac{D ^ 2} {2!} + \cdots \right) U = U ^ \dagger \begin{Bmatrix}\exp (d _{11}) & 0 & \cdots & 0\\\\ 0 & \exp (d _{22}) & \cdots & 0\\\\ \vdots & \vdots & \ddots & \vdots\\\\ 0 & 0 & \cdots & \exp (d _ {NN}) \end{Bmatrix} U. $ $

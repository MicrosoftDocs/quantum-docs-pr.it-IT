...
Qualsiasi vettore di colonna bidimensionale di numeri reali o complessi con Norm $1 $ rappresenta un possibile stato quantum utilizzato da un qubit. Quindi $ \begin{Bmatrix} \Alpha \\ \\ \beta \end{Bmatrix} $ rappresenta uno stato qubit se $ \Alpha $ e $ \beta $ sono numeri complessi soddisfacenti $ | \Alpha | ^ 2 + | \beta | ^ 2 = $1. Alcuni esempi di vettori di stato quantum validi che rappresentano qubits includono
...
Sebbene si tratta di un esempio semplice (poiché l' <xref:microsoft.quantum.intrinsic.h> operazione è autocontigua), è possibile vedere come questa operazione diventa inestimabile per operazioni qubit più complesse.

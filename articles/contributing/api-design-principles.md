...
- ✅**Esporre la** logica classica puramente deterministica come funzioni anziché operazioni.
  ...
  *Esempi:*
  - Una subroutine che consente di eseguire la scrittura in modo deterministico dei quadrati dell'input a virgola mobile e deve quindi essere esposta `Squared : Double -> Double` all'utente come anziché come `Square : Double => Double`operazione. Questo consente la chiamata della subroutine in più posizioni, ad esempio all'interno di altre funzioni, e fornisce informazioni di ottimizzazione utili al compilatore che possono influire sulle prestazioni e sulle ottimizzazioni.
  - `ForEach<'TInput, 'TOutput>('TInput => 'TOutput, 'TInput[]) => 'TOutput[]`e `Mapped<'TInput, 'TOutput>('TInput -> 'TOutput, 'TInput[]) -> 'TOutput[]` differiscono nelle garanzie effettuate per quanto riguarda il determinismo; entrambi sono utili in circostanze diverse.
...
- ✅**Generalizzare** il tipo di input quanto più ragionevole per ogni funzione e operazione, usando i parametri di tipo in base alle esigenze.
  ...
  *Esempi:*
  - `ApplyToEach`il tipo `<'T>(('T => Unit), 'T[]) => Unit` è anziché il tipo specifico dell'applicazione più comune `((Qubit => Unit), Qubit[]) => Unit`.
...
- ✅**Usare i** tipi di tupla per raggruppare in modo logico input e output che sono significativi solo se considerati insieme. In questi casi è consigliabile utilizzare un tipo definito dall'utente.
  ...
  *Esempi:*
  - Una funzione per l'output del valore minimo locale di un'altra funzione potrebbe dover prendere i limiti di un intervallo di ricerca come input, `LocalMinima(fn : (Double -> Double), (left : Double, right : Double)) : Double` in modo che possa essere una firma appropriata.
...
- ✅**Introdurre nuovi** tipi definiti dall'utente per indicare che un determinato tipo di base deve essere utilizzato solo in un senso molto particolare.
  ...
  *Esempi:*
  - Un'operazione che deve essere interpretata in modo specifico come operazione che codifica i dati classici in un registro quantum può essere appropriata per l'etichetta con un tipo `newtype InputEncoder = (Apply : (Qubit[] => Unit))`definito dall'utente.
...
- ✅**Introdurre nuovi** tipi definiti dall'utente con elementi denominati che consentono l'estendibilità futura (ad esempio, una struttura dei risultati che può contenere altri elementi denominati in futuro).
  ...
  *Esempi:*
  - Quando un'operazione `TrainModel` espone un numero elevato di opzioni di configurazione, l'esposizione di queste opzioni come `TrainingOptions` un nuovo tipo definito dall'utente `DefaultTrainingOptions : Unit -> TrainingOptions` e la creazione di una nuova funzione consentono agli utenti di eseguire l'override di elementi denominati specifici nei valori UDT TrainingOptions, consentendo agli sviluppatori di librerie di aggiungere nuovi elementi UDT nel modo appropriato.
...
- ✅Per **dichiarare gli** elementi denominati per i nuovi tipi definiti dall'utente, è preferibile che gli utenti conoscano la corretta decostruzione della tupla.
  ...
  *Esempi:*
  - Quando si rappresenta un numero complesso nella relativa scomposizione polare, `newtype ComplexPolar = (Magnitude: Double, Argument: Double)` preferire `newtype ComplexPolar = (Double, Double)`a.
...
- ⛔️ **non** introducono tipi definiti dall'utente che richiedono l'uso frequente dell'operatore Unwrap (`!`) o che richiedono in genere più livelli di annullamento del wrapping. Le strategie di mitigazione possibili includono:
  ...
  - Quando si espone un tipo definito dall'utente con un singolo elemento, è consigliabile definire un nome per l'elemento. Si consideri `newtype Encoder = (Apply : (Qubit[] => Unit is Adj + Ctl))` , ad esempio `newtype Encoder = (Qubit[] => Unit is Adj + Ctl)`, la preferenza a.
...
- ⛔️ **non** introducono nuovi tipi definiti dall'utente che duplicano i tipi incorporati senza fornire un'espressività aggiuntiva.
  ...
  *Esempi:*
  - Un tipo `newtype QubitRegister = Qubit[]` definito dall'utente non fornisce alcuna `Qubit[]`espressività aggiuntiva ed è quindi più difficile da usare senza alcun vantaggio distinguibile.
  - Un tipo `newtype LittleEndian = Qubit[]` definito dall'utente documenta il modo in cui deve essere usato e interpretato il registro sottostante e quindi fornisce un'espressività aggiuntiva sul relativo tipo di base.
...
- ⛔️ **non** introducono funzioni di accesso a meno che non sia strettamente necessario;   in questo caso, è consigliabile preferire gli elementi denominati.
  ...
  *Esempi:*
  - Quando si introduce un `newtype Complex = (Double, Double)`tipo definito dall'utente, è `newtype Complex = (Real : Double, Imag : Double)` preferibile `GetReal : Complex -> Double` modificare `GetImag : Complex -> Double`la definizione in per introdurre funzioni e.
...
- ✅**Assegnare un** nome agli spazi `Publisher.Product.DomainArea`dei nomi come.

  *Esempi:*
  - Le funzioni, le operazioni e i tipi definiti dall'utente pubblicati da Microsoft nell'ambito della funzionalità di simulazione quantistica del quantum Development Kit vengono `Microsoft.Quantum.Simulation` inseriti nello spazio dei nomi.
...
- ✅**Inserire le** operazioni, le funzioni e i tipi definiti dall'utente utilizzati solo all'interno di domini specifici in spazi dei nomi indicanti il dominio di utilità. Se necessario, utilizzare i sottospazi dei nomi per indicare le attività mirate all'interno di ogni spazio dei nomi specifico del dominio.
  ...
  *Esempi:*
  - La libreria Quantum Machine Learning pubblicata da Microsoft viene posizionata in gran parte @"microsoft.quantum.machinelearning" nello spazio dei nomi, ma i @"microsoft.quantum.machinelearning.datasets" set di impostazioni di esempio vengono forniti dallo spazio dei nomi.
  - Le API di chimica quantistica pubblicate da Microsoft come parte del quantum Development Kit devono essere inserite in `Microsoft.Quantum.Chemistry`. Le funzionalità specifiche dell'implementazione della scomposizione di Jordan--Wigner possono essere inserite `Microsoft.Quantum.Chemistry.JordanWigner`in, in modo che l'interfaccia primaria per l'area del dominio di chimica quantistica non sia interessata dalle implementazioni.
...
- ✅Ogni volta che è **ragionevole, inserire** tutte le funzioni e le operazioni necessarie per implementare un'API nello stesso spazio dei nomi dell'API implementata, ma contrassegnata con le parole chiave "private" o "Internal" per indicare che non fanno parte della superficie dell'API pubblica per una raccolta. Usare un nome che inizia con un carattere di`_`sottolineatura () per distinguere visivamente le operazioni e le operazioni interne e private dalle funzioni chiamabili pubbliche.

  *Esempi:*
  - Il nome `_Features` dell'operazione indica una funzione privata per uno spazio dei nomi e un assembly specificati e deve essere accompagnata dalla `internal` parola chiave.

- ✅Nel raro caso in cui sia necessario un ampio set di funzioni o operazioni private per implementare l'API per un determinato spazio dei **nomi,** inserirle in un nuovo spazio dei nomi che corrisponda allo `.Private`spazio dei nomi implementato e che termina con.
...
- ✅**Scegliere le** operazioni e i nomi di funzione che comunicano chiaramente l'effetto desiderato di un oggetto chiamabile e non la relativa implementazione. Si noti che l'implementazione può e deve essere documentata nei [commenti della documentazione API](xref:microsoft.quantum.language.statements#documentation-comments).
